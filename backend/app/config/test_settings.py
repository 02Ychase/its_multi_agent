...
def main():
    ...
    print("\n[2] AI 服务配置检查")
    print(f"{'Main Model Key':<20}: {mask_secret(settings.MAIN_API_KEY)}") # 左对齐 占 20 个字符宽度。
    print(f"{'Main Model URL':<20}: {settings.MAIN_BASE_URL or '未配置'}")
    print(f"{'Sub Model Key':<20}: {mask_secret(settings.SUB_API_KEY)}")
    print(f"{'Sub Model URL':<20}: {settings.SUB_BASE_URL or '未配置'}")
